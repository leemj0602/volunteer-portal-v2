                ...
                {!currRegistrations?.length ? <tr>
                    ...
                    return <tr key={index} className={cancelledByOrganisation ? "bg-gray-200" : ""}>
                        ...
                        <Cell className={`whitespace-nowrap ${cancelledByOrganisation ? "text-gray-400" : ""}`}>
                            {trainingSchedule.training.subject && <button className={!cancelledByOrganisation ? "text-secondary hover:text-primary cursor-pointer" : ""} disabled={cancelledByOrganisation} onClick={() => navigate(`/volunteer/trainings/${trainingSchedule.training.id}`)}>
