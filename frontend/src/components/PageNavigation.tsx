...
export default function PageNavigation({ page, pages, limit, array, previousPage, nextPage }: PageNavigationProps) {
    return <div className="flex justify-between items-center mt-4">
        <span className="text-gray-500">Showing {page + (page * limit) + 1} - {page + ((page + 1) * limit) > array.length ? array.length : page + ((page + 1) * limit)} of {array.length} {inflect("entries", array.length)} </span>
